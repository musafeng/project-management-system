...
interface SubcontractPaymentDetail extends SubcontractPayment {
  projectId?: string
  vendorId?: string
  paymentMethod?: string | null
  paymentNumber?: string | null
...
export default function SubcontractPaymentsPage() {
  ...
  const [detailData, setDetailData] = useState<SubcontractPaymentDetail | null>(null)
  ...
  const detailCacheRef = useRef<Record<string, SubcontractPaymentDetail>>({})
  const pageCacheRef = useRef<Record<number, SubcontractPayment[]>>({})
  ...
  const loadPayments = async (
    ...
    try {
      ...
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<SubcontractPayment>(result.data, nextPage, nextPageSize)
  ...
  const handleEditClick = async (id: string) => {
    try {
      ...
      const result: ApiResponse<SubcontractPaymentDetail> = await response.json()
  ...
  const prefetchPaymentPage = async (targetPage: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/subcontract-payments?${params.toString()}`)
      const result: ApiResponse<any> = await response.json()
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<SubcontractPayment>(result.data, targetPage, pageSize)
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.success) {
        message.success(getDeleteSuccessMessage('分包付款'))
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const response = await fetch(editingId ? `/api/subcontract-payments/${editingId}` : '/api/subcontract-payments', {
      ...
      if (result.success) {
        message.success(getSaveSuccessMessage('分包付款', Boolean(editingId)))
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportUrl = params.toString() ? `/api/subcontract-payments/export?${params.toString()}` : '/api/subcontract-payments/export'
      await downloadExportFile(exportUrl, `分包付款台账_${dayjs().format('YYYYMMDD_HHmmss')}.csv`, '导出失败')
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Popconfirm title={getDeleteConfirmTitle('分包付款')} description={getDeleteConfirmDescription('分包付款')} onConfirm={() => handleDelete(record.id)} okText="确定" cancelText="取消" disabled={isApprovalReadonly(record.approvalStatus)}>
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            <Space wrap style={{ width: '100%' }}>
              <Input
                placeholder="搜索合同名称 / 编号 / 项目 / 分包单位 / 付款单号"
          ...
          <Table<SubcontractPayment>
            ...
            locale={{
              emptyText: getLedgerEmptyText('分包付款记录', '新增付款'),
            }}
          />
        </div>
      </div>
        ...
        >
          ...
          {selectedContract ? (
            <div style={{ marginBottom: 16, padding: '10px 12px', background: '#fafafa', border: '1px solid #f0f0f0', borderRadius: 6, color: '#595959' }}>
              项目：{selectedContract.projectName}；分包单位：{selectedContract.subcontractVendorName}；合同名称：{selectedContract.name || '—'}；未付金额：{fmtMoney(selectedContract.unpaidAmount)}
            </div>
          ) : null}
      ...
      <DetailModal
        title={detailData ? `分包付款详情 · ${detailData.paymentNumber || detailData.contractCode}` : '分包付款详情'}
        open={detailOpen}
        ...
        extra={detailData ? (
          <LedgerDetailExtra
            ...
            summaryHint={joinDetailHints(
              ...
              getApprovalActionBoundaryHint(detailData.approvalStatus, '分包付款单'),
            ...
            actionBar={(
              <Space size="small" wrap>
                <Button
                  size="small"
                  type="link"
                  onClick={() => window.open(buildLedgerRouteHref('/subcontract-contracts', {
