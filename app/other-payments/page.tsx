...
export default function OtherPaymentsPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/other-payments')
  ...
  const handleSubmit = async (values: any) => {
    const payload = { ...values, paymentDate: values.paymentDate?.format('YYYY-MM-DD') }
  ...
  const handleDelete = async (id: string) => {
    const res = await fetch(`/api/other-payments/${id}`, { method: 'DELETE' })
    const json = await res.json()
    if (json.success) { message.success('已删除'); load() }
    else message.error(json.error || '删除失败')
  }

  const total = data.reduce((s, r) => s + Number(r.paymentAmount), 0)
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, padding: 20, minHeight: '80vh' }}>
      ...
      <Modal title={editing ? '编辑其他付款' : '新增其他付款'} open={modalOpen}
