...
export default function OtherPaymentsPage() {
  ...
  const total = useMemo(() => data.reduce((s, r) => s + Number(r.paymentAmount), 0), [data])
  ...
  const load = async (params?: { keyword?: string; startDate?: string; endDate?: string; projectId?: string }) => {
    ...
    try {
      ...
      const url = queryString ? `/api/other-payments?${queryString}` : '/api/other-payments'
      const result = await requestApi<OtherPayment[]>(url, undefined, '加载其他付款失败')
      if (result.success) {
        setData(result.data)
      } else {
  ...
  useEffect(() => {
    ...
    loadFormFields('other-payments').then((result) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const result = await requestAction(`/api/other-payments/${id}`, { method: 'DELETE' }, '删除失败')
      if (result.success) {
        message.success('已删除')
        load()
      } else {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportUrl = queryString ? `/api/other-payments/export?${queryString}` : '/api/other-payments/export'
      ...
      a.href = downloadUrl
      a.download = `其他付款_${dayjs().format('YYYYMMDD_HHmmss')}.csv`
      a.click()
  ...
  const columns: ColumnsType<OtherPayment> = [
    ...
    { title: '付款事由', dataIndex: 'paymentType', width: 150 },
    { title: '金额', dataIndex: 'paymentAmount', width: 120, align: 'right', render: v => <span style={{ color: '#ff4d4f', fontWeight: 600 }}>{fmt(v)}</span> },
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, padding: 20, minHeight: '80vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ color: '#ff4d4f', fontWeight: 600 }}>金额合计：{fmt(total)}</span>
      </div>
      ...
      <Modal title={editing ? '编辑其他付款' : '新增其他付款'} open={modalOpen} onOk={() => form.submit()} onCancel={() => handleCloseModal(false)} afterClose={handleModalAfterClose} confirmLoading={submitting} okButtonProps={{ disabled: submitting }} cancelButtonProps={{ disabled: submitting }} width={560} okText="确定" cancelText="取消">
        <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
          ...
          {formConfigStatus === 'ready' ? (
            <DynamicForm
              ...
              form={form}
              disabled={false}
              validationScope="other-payments"
