...
export default function SubcontractContractsPage() {
  ...
  const [detailData, setDetailData] = useState<SubcontractContractDetail | null>(null)
  ...
  const detailCacheRef = useRef<Record<string, SubcontractContractDetail>>({})
  const pageCacheRef = useRef<Record<number, SubcontractContract[]>>({})
  ...
  const loadContracts = async (
    ...
    try {
      ...
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<SubcontractContract>(result.data, nextPage, nextPageSize)
  ...
  const handleEditClick = async (id: string) => {
    try {
      const response = await fetch(`/api/subcontract-contracts/${id}`)
      const result: ApiResponse<SubcontractContractDetail> = await response.json()
      ...
      if (result.success && result.data) {
        ...
        form.setFieldsValue({
          ...
          subcontractType: result.data.subcontractType || undefined,
  ...
  const prefetchContractPage = async (targetPage: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/subcontract-contracts?${params.toString()}`)
      const result: ApiResponse<any> = await response.json()
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<SubcontractContract>(result.data, targetPage, pageSize)
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const payload = {
        ...
        subcontractType: values.subcontractType || null,
      ...
      if (result.success) {
        message.success(getSaveSuccessMessage('分包合同', Boolean(editingId)))
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportUrl = params.toString() ? `/api/subcontract-contracts/export?${params.toString()}` : '/api/subcontract-contracts/export'
      await downloadExportFile(exportUrl, `分包合同台账_${dayjs().format('YYYYMMDD_HHmmss')}.csv`, '导出失败')
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Popconfirm title={getDeleteConfirmTitle('分包合同')} description={getDeleteConfirmDescription('分包合同')} onConfirm={() => handleDelete(record.id)} okText="确定" cancelText="取消" disabled={isApprovalReadonly(record.approvalStatus)}>
    ...
    >
      ...
      >
        ...
        >
          ...
          <Table<SubcontractContract>
            ...
            locale={{
              emptyText: getLedgerEmptyText('分包合同', '新增合同'),
            }}
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <Form.Item label="分包类型" name="subcontractType">
            <Input placeholder="请输入分包类型" />
          </Form.Item>
      ...
      <DetailModal
        title={detailData ? `分包合同详情 · ${detailData.code}` : '分包合同详情'}
        open={detailOpen}
        ...
        items={[
          ...
          { key: 'subcontractVendorName', label: '分包单位', value: getDisplayText(detailData?.subcontractVendorName) },
          ...
          { key: 'subcontractType', label: '分包类型', value: getDisplayText(detailData?.subcontractType) },
        ...
        extra={detailData ? (
          <LedgerDetailExtra
                ...
                value: (
                  <Space size={4} wrap>
                    ...
                    <Button
                      size="small"
                      type="link"
                      onClick={() => window.open(buildLedgerRouteHref('/subcontract-payments', {
            ...
            summaryHint={joinDetailHints(
              ...
              getApprovalActionBoundaryHint(detailData.approvalStatus, '分包合同'),
              ...
              <Space size="small" wrap>
                {detailData.recentFlows?.[0]?.id ? (
                  <Button
                    size="small"
                    type="link"
                    onClick={() => { window.location.href = buildLedgerRouteHref('/subcontract-payments', {
                      contractId: detailData.id,
                      projectId: detailData.projectId,
                      detailId: detailData.recentFlows?.[0]?.id,
                      returnPath: buildLedgerRouteHref('/subcontract-contracts', {
                        projectId,
                        keyword,
                        approvalStatus,
            ...
            onFlowClick={(flow) => {
              if (flow.id) {
                window.location.href = buildLedgerRouteHref('/subcontract-payments', {
                  contractId: detailData.id,
                  projectId: detailData.projectId,
                  detailId: flow.id,
                  returnPath: buildLedgerRouteHref('/subcontract-contracts', {
