...
const handler = apiHandlerWithMethod({
  ...
  GET: async (req) => {
    ...
    const recentPayments = await db.laborPayment.findMany({
  ...
  PUT: async (req) => {
    ...
    if (body.laborWorkerId !== undefined) {
      if (typeof body.laborWorkerId !== 'string' || body.laborWorkerId.trim() === '') {
        throw new BadRequestError('劳务人员不能为空')
      }
      const worker = await db.laborWorker.findUnique({ where: { id: body.laborWorkerId }, select: { id: true } })
      if (!worker) {
    ...
    if (body.laborType !== undefined) {
      updateData.laborType = body.laborType?.trim() || null
    }
  ...
  DELETE: async (req) => {
    ...
    return success({ message: '劳务合同已删除' })
  },
})
