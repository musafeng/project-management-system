...
export const { GET } = apiHandlerWithPermissionAndLog({
  GET: async (req) => {
    ...
    const canExport = canAccessApi(user.systemRole, 'POST', '/api/sales-expenses')
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent('sales-expenses.csv')}`,
      },
    })
  },
