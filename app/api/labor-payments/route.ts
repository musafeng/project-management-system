  ...
  GET: async (req) => {
    ...
    if (keyword) {
      where.OR = [
        ...
        { contract: { worker: { name: { contains: keyword } } } },
      ]
    }
    ...
    const [total, paymentSummary, relatedContractIds, payments] = await Promise.all([
      pagination.paginated ? db.laborPayment.count({ where }) : Promise.resolve(0),
    ...
    const relatedContractSummary =
      pagination.paginated && relatedContractIds.length > 0
        ? await db.laborContract.aggregate({
