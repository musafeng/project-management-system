...
const handler = apiHandlerWithMethod({
  ...
  GET: async (req) => {
    ...
    const relatedPayments = await db.subcontractPayment.findMany({
    ...
    const approvalTrail = await getApprovalTimeline('subcontractPayment', payment.id)
  ...
  /**
   * PUT /api/subcontract-payments/{id}
   * 更新付款记录并回写合同汇总
   */
  PUT: async (req) => {
    const id = req.url.split('/').pop()
    ...
    const existing = await db.subcontractPayment.findUnique({
      where: { id },
      select: {
        id: true,
        contractId: true,
    ...
    const nextContract = await db.subcontractContract.findUnique({
    ...
    const oldContract = existing.contractId === nextContractId
      ? nextContract
      : await db.subcontractContract.findUnique({
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const record = await tx.subcontractPayment.update({
        where: { id },
        data: {
          contractId: nextContractId,
          projectId: nextContract.projectId,
          vendorId: nextContract.vendorId,
    ...
    return success({
      ...
      subcontractVendorName: updated.contract.vendor.name,
