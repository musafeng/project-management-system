...
const handler = apiHandlerWithMethod({
  ...
  GET: async (req) => {
    ...
    const recentPayments = await db.subcontractPayment.findMany({
  ...
  PUT: async (req) => {
    ...
    if (body.subcontractVendorId !== undefined) {
      if (typeof body.subcontractVendorId !== 'string' || body.subcontractVendorId.trim() === '') {
        throw new BadRequestError('分包单位不能为空')
      }
      const vendor = await db.subcontractVendor.findUnique({ where: { id: body.subcontractVendorId }, select: { id: true } })
      if (!vendor) {
    ...
    if (body.subcontractType !== undefined) {
      updateData.subcontractType = body.subcontractType?.trim() || null
    }
