...
type ProcurementContractSelect = Awaited<ReturnType<typeof db.procurementContract.findMany>>[number]
...
export const { GET, POST } = apiHandlerWithPermissionAndLog({
  ...
  GET: async (req) => {
    ...
    // 转换返回格式
    const result = contracts.map((contract: ProcurementContractSelect) => ({
      id: contract.id,
      code: contract.code,
