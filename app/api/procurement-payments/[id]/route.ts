...
const handler = apiHandlerWithMethod({
  ...
  GET: async (req) => {
    ...
    const relatedPayments = await db.procurementPayment.findMany({
    ...
    const approvalTrail = await getApprovalTimeline('procurementPayment', payment.id)
  ...
  PUT: async (req) => {
    ...
    const existing = await db.procurementPayment.findUnique({
      where: { id },
      select: {
        id: true,
        contractId: true,
    ...
    const nextContract = await db.procurementContract.findUnique({
    ...
    const oldContract = existing.contractId === nextContractId
      ? nextContract
      : await db.procurementContract.findUnique({
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const record = await tx.procurementPayment.update({
        where: { id },
        data: {
      ...
      if (existing.contractId === nextContractId) {
        ...
        await tx.procurementContract.update({
          where: { id: nextContractId },
          data: {
    ...
    return success({
      ...
      supplierName: updated.contract.supplier.name,
