...
const MODEL = 'procurementPayment' as const
const BASE = '/api/procurement-payments'
...
/**
 * 采购付款 submit - 使用 BaseSettlementService 包裹
 * 
 * 流程：
 * 1. 验证付款记录存在
...
async function submitWithSettlementService(id: string): Promise<void> {
  try {
    await BaseSettlementService.executeInTransaction(async (context) => {
      ...
      // 4. 刷新合同金额
      await BaseSettlementService.refreshAmounts('ProcurementPayment', payment.contractId, context)

      // 5. 记录审计日志
      await BaseSettlementService.logAudit('CREATE', 'ProcurementPayment', id, {
