...
export default function SalesExpensesPage() {
  ...
  const load = async (params?: { keyword?: string; startDate?: string; endDate?: string }) => {
    ...
    try {
      ...
      const url = queryString ? `/api/sales-expenses?${queryString}` : '/api/sales-expenses'
      const result = await requestApi<Expense[]>(url, undefined, '加载销售费用报销失败')
      if (result.success) {
        setData(result.data)
      } else {
  ...
  useEffect(() => {
    ...
    loadFormFields('sales-expenses').then((result) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const result = await requestAction(`/api/sales-expenses/${id}`, { method: 'DELETE' }, '删除失败')
      if (result.success) {
        message.success('已删除')
        load()
      } else {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportUrl = queryString ? `/api/sales-expenses/export?${queryString}` : '/api/sales-expenses/export'
      ...
      a.href = downloadUrl
      a.download = `销售费用报销_${dayjs().format('YYYYMMDD_HHmmss')}.csv`
      a.click()
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, padding: 20, minHeight: '80vh' }}>
      ...
      <Modal title={editing ? '编辑销售费用报销' : '新增销售费用报销'} open={modalOpen} onOk={() => form.submit()} onCancel={() => handleCloseModal(false)} afterClose={handleModalAfterClose} confirmLoading={submitting} okButtonProps={{ disabled: submitting }} cancelButtonProps={{ disabled: submitting }} width={560} okText="确定" cancelText="取消">
        <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
          ...
          {formConfigStatus === 'ready' ? (
            <DynamicForm
              ...
              form={form}
              disabled={false}
              validationScope="sales-expenses"
