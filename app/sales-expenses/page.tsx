...
export default function SalesExpensesPage() {
  ...
  const mobileCards = (
    <MobileCardList<Expense>
      ...
      actions={(record) => {
        ...
        return (
          <Space size="small" wrap>
            ...
            {canDelete ? (
              <Popconfirm title="删除销售费用报销" description="确定删除该报销记录吗？" onConfirm={() => void handleDelete(record.id)} okText="确定" cancelText="取消" disabled={locked}>
      ...
      empty={<EmptyHint title="暂无销售费用报销数据" desc="新增销售费用报销后，可在此管理审批进度。" />}
    />
  )
