...
export default function ManagementExpensesPage() {
  ...
  const load = async () => {
    setLoading(true)
    try { const res = await fetch('/api/management-expenses'); const j = await res.json(); if (j.success) setData(j.data) }
  ...
  const handleOpen = (record?: Expense) => {
    ...
    const its = record?.expenseItems?.length ? record.expenseItems : [{ type: '办公费', amount: 0 }]
    setItems(its)
    form.resetFields()
  ...
  const handleDelete = async (id: string) => {
    const res = await fetch(`/api/management-expenses/${id}`, { method: 'DELETE' })
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, padding: 20, minHeight: '80vh' }}>
      ...
      <Modal title={editing ? '编辑管理费用报销' : '新增管理费用报销'} open={modalOpen} onOk={() => form.submit()} onCancel={() => setModalOpen(false)} width={580} okText="确定" cancelText="取消">
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {items.map((item, idx) => (
              <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                <Select value={item.type} onChange={v => setItems(its => its.map((x, i) => i === idx ? { ...x, type: v } : x))} options={EXPENSE_TYPES.map(t => ({ label: t, value: t }))} style={{ width: 130 }} />
            ...
            <Button type="dashed" icon={<PlusOutlined />} size="small" onClick={() => setItems(its => [...its, { type: '办公费', amount: 0 }])}>添加明细</Button>
          </div>
