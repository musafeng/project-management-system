...
export default function ManagementExpensesPage() {
  ...
  const load = async (filters: FilterValues = {}) => {
    ...
    const url = `/api/management-expenses${params.toString() ? `?${params.toString()}` : ''}`
  ...
  const handleDelete = async (id: string) => {
    const result = await requestApi(`/api/management-expenses/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    ...
    <FilterBar
      ...
      extra={
        <Button
          ...
          style={{ color: '#8c8c8c' }}
          onClick={() => {
            const params = new URLSearchParams({ resourceType: 'management-expenses' })
        ...
        actions={(record) => {
          ...
          return (
            <Space size="small" wrap>
              ...
              {canDelete ? (
                <Popconfirm title="删除管理费用报销" description="确定删除该报销记录吗？" onConfirm={() => handleDelete(record.id)} okText="确定" cancelText="取消" disabled={locked}>
