...
function applyClientFilters(data: ProcurementPayment[], filters: FilterValues) {
  let filtered = data
...
export default function ProcurementPaymentsPage() {
  const [data, setData] = useState<ProcurementPayment[]>([])
  ...
  const loadContracts = async () => {
    ...
    const result = await requestApi<ProcurementContract[]>('/api/procurement-contracts', {
      credentials: 'include',
      fallbackError: '加载采购合同列表失败',
    })
  ...
  const load = async (filters: FilterValues = {}) => {
    ...
    const result = await requestApi<ProcurementPayment[]>(url, {
      credentials: 'include',
  ...
  const handleSubmit = async (values: any) => {
    ...
    const result = await requestApi('/api/procurement-payments', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    const result = await requestApi(`/api/procurement-payments/${id}`, {
      method: 'DELETE',
      credentials: 'include',
  ...
  const columns: ColumnsType<ProcurementPayment> = [
    ...
    { title: '供应商', dataIndex: 'supplierName', width: 130, render: (v) => v || '-' },
    {
      title: '付款金额',
      dataIndex: 'amount',
      width: 130,
    ...
    <FilterBar
      ...
      extra={
        <Button
          ...
          style={{ color: '#8c8c8c' }}
          onClick={() => {
            const params = new URLSearchParams({ resourceType: 'procurement-payments' })
  ...
  const mobileCards = (
    <>
      <MobileCardList<ProcurementPayment>
        ...
        loading={loading}
        getKey={(item) => item.id}
        getTitle={(item) => item.contractName || '采购付款'}
        ...
        >
          ...
          {selectedContract && (
            ...
            >
              ...
              <div>供应商：{selectedContract.supplierName || '-'}</div>
              <div>联系电话：{selectedContract.supplierPhone || '-'}</div>
              <div>银行卡：{selectedContract.supplierBankAccount || '-'}</div>
              <div>开户行：{selectedContract.supplierBankName || '-'}</div>
            </div>
          )}
