...
interface ProcurementPaymentDetail extends ProcurementPayment {
  projectId?: string
  paymentMethod?: string | null
  paymentNumber?: string | null
...
export default function ProcurementPaymentsPage() {
  ...
  const [detailData, setDetailData] = useState<ProcurementPaymentDetail | null>(null)
  ...
  const detailCacheRef = useRef<Record<string, ProcurementPaymentDetail>>({})
  const pageCacheRef = useRef<Record<number, ProcurementPayment[]>>({})
  ...
  const loadContracts = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.error('加载采购合同列表失败:', result.error)
      }
    } catch (err) {
  ...
  const loadPayments = async (
    ...
    try {
      ...
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<ProcurementPayment>(result.data, nextPage, nextPageSize)
  ...
  const handleEditClick = async (id: string) => {
    try {
      ...
      const result: ApiResponse<ProcurementPaymentDetail> = await response.json()
  ...
  const prefetchPaymentPage = async (targetPage: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/procurement-payments?${params.toString()}`)
      const result: ApiResponse<any> = await response.json()
      if (result.success && result.data) {
        const normalized = normalizePaginatedList<ProcurementPayment>(result.data, targetPage, pageSize)
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const response = await fetch(editingId ? `/api/procurement-payments/${editingId}` : '/api/procurement-payments', {
      ...
      if (result.success) {
        message.success(getSaveSuccessMessage('采购付款', Boolean(editingId)))
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportUrl = params.toString() ? `/api/procurement-payments/export?${params.toString()}` : '/api/procurement-payments/export'
      await downloadExportFile(exportUrl, `采购付款台账_${dayjs().format('YYYYMMDD_HHmmss')}.csv`, '导出失败')
   ...
   * 表格列定义
   */
  const columns: ColumnsType<ProcurementPayment> = [
    {
      title: '合同编号',
    ...
    },
    {
      ...
      width: 130,
      align: 'right',
      render: (value: number) => <span style={{ color: '#ff7a45', fontWeight: 600 }}>{fmtMoney(value)}</span>,
    },
    {
      title: '付款日期',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Popconfirm
            title={getDeleteConfirmTitle('采购付款')}
            description={getDeleteConfirmDescription('采购付款')}
            onConfirm={() => handleDelete(record.id)}
            okText="确定"
            cancelText="取消"
            disabled={isApprovalReadonly(record.approvalStatus)}
          >
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            <Space wrap style={{ width: '100%' }}>
              <Input
                placeholder="搜索合同名称 / 编号 / 项目 / 供应商 / 付款单号"
          ...
          <Table<ProcurementPayment>
            ...
            locale={{
              emptyText: getLedgerEmptyText('采购付款记录', '新增付款'),
            }}
          />
        </div>
      </div>
        ...
        >
          ...
          {selectedContract ? (
            <div style={{ marginBottom: 16, padding: '10px 12px', background: '#fafafa', border: '1px solid #f0f0f0', borderRadius: 6, color: '#595959' }}>
              项目：{selectedContract.projectName}；供应商：{selectedContract.supplierName}；合同名称：{selectedContract.name}；未付金额：{fmtMoney(selectedContract.unpaidAmount)}
            </div>
          ) : null}
      ...
      <DetailModal
        title={detailData ? `采购付款详情 · ${detailData.paymentNumber || detailData.contractCode}` : '采购付款详情'}
        open={detailOpen}
        ...
        extra={detailData ? (
          <LedgerDetailExtra
            ...
            summaryHint={joinDetailHints(
              ...
              getApprovalActionBoundaryHint(detailData.approvalStatus, '采购付款单'),
            ...
            actionBar={(
              <Space size="small" wrap>
                <Button
                  size="small"
                  type="link"
                  onClick={() => window.open(buildLedgerRouteHref('/procurement-contracts', {
