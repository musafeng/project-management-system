...
export default function OtherReceiptsPage() {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      console.error('提交其他收款失败:', err)
      message.error('提交失败，请检查表单后重试')
    }
  }

  const handleDelete = async (id: string) => {
    const response = await fetch(`/api/other-receipts/${id}`, { method: 'DELETE' })
    const json = await response.json()
  ...
  const columns: ColumnsType<OtherReceipt> = [
    { title: '收款事由', dataIndex: 'receiptType', width: 180 },
    {
      ...
      width: 120,
      align: 'right',
      render: (value) => <span style={{ color: '#52c41a', fontWeight: 600 }}>{fmt(Number(value))}</span>,
    },
