# 采购付款 API（ProcurementPayment）开发完成总结
...
| 文件 | 行数 | 功能 |
|------|------|------|
| `route.ts` | 156 | GET 列表、POST 创建 |
| `[id]/route.ts` | 126 | GET 详情、DELETE 删除 |
...
| # | 方法 | 路径 | 功能 |
|---|------|------|------|
| 1 | GET | `/api/procurement-payments` | 获取采购付款列表 |
| 2 | POST | `/api/procurement-payments` | 创建采购付款 |
| 3 | GET | `/api/procurement-payments/{id}` | 获取付款详情 |
| 4 | DELETE | `/api/procurement-payments/{id}` | 删除采购付款 |

---

## ✅ 3. 每个接口的请求示例

### 1️⃣ GET /api/procurement-payments - 获取列表
...
# 按合同过滤
curl "http://localhost:3000/api/procurement-payments?contractId=CONTRACT_ID"

# 按项目过滤
curl "http://localhost:3000/api/procurement-payments?projectId=PROJECT_ID"

# 组合过滤
curl "http://localhost:3000/api/procurement-payments?contractId=CONTRACT_ID&projectId=PROJECT_ID"
```

### 2️⃣ POST /api/procurement-payments - 创建付款
...
### 3️⃣ GET /api/procurement-payments/{id} - 获取详情

```bash
curl http://localhost:3000/api/procurement-payments/PAYMENT_ID
```

### 4️⃣ DELETE /api/procurement-payments/{id} - 删除付款

```bash
curl -X DELETE http://localhost:3000/api/procurement-payments/PAYMENT_ID
```
  ...
  "data": [
    {
      ...
      "contractCode": "PROC1710710400000",
      "projectName": "项目 A",
      "supplierName": "供应商 A",
      "amount": 50000,
      "paymentDate": "2026-03-17T10:00:00Z",
    ...
    },
    {
      ...
      "contractCode": "PROC1710710500000",
      "projectName": "项目 B",
      "supplierName": "供应商 B",
      "amount": 30000,
...
✅ **已验证**:
- Prisma 客户端已生成
- ProcurementPayment 表已配置
- 所有关联关系已定义：
  - `contract` → ProcurementContract (多对一)
  - `project` → Project (多对一)
- 所有外键约束已启用
- 所有索引已创建
...
├── contractId: String (外键 → ProcurementContract)
