# 采购合同 API（ProcurementContract）开发完成总结
...
| # | 方法 | 路径 | 功能 |
|---|------|------|------|
| 1 | GET | `/api/procurement-contracts` | 获取采购合同列表 |
| 2 | POST | `/api/procurement-contracts` | 创建采购合同 |
| 3 | GET | `/api/procurement-contracts/{id}` | 获取合同详情 |
| 4 | PUT | `/api/procurement-contracts/{id}` | 更新合同信息 |
| 5 | DELETE | `/api/procurement-contracts/{id}` | 删除采购合同 |

---

## ✅ 3. 每个接口的请求示例

### 1️⃣ GET /api/procurement-contracts - 获取列表
...
# 按项目过滤
curl "http://localhost:3000/api/procurement-contracts?projectId=PROJECT_ID"

# 按施工立项过滤
curl "http://localhost:3000/api/procurement-contracts?constructionId=CONSTRUCTION_ID"

# 组合过滤
curl "http://localhost:3000/api/procurement-contracts?projectId=PROJECT_ID&constructionId=CONSTRUCTION_ID"
```

### 2️⃣ POST /api/procurement-contracts - 创建合同
...
curl -X POST http://localhost:3000/api/procurement-contracts \
  ...
  -d '{
    ...
    "supplierId": "clp2b3c4d5e6f7g8h9i0j1k2l",
...
### 3️⃣ GET /api/procurement-contracts/{id} - 获取详情

```bash
curl http://localhost:3000/api/procurement-contracts/CONTRACT_ID
```

### 4️⃣ PUT /api/procurement-contracts/{id} - 更新合同

```bash
curl -X PUT http://localhost:3000/api/procurement-contracts/CONTRACT_ID \
  -H "Content-Type: application/json" \
  -d '{
    "changedAmount": 10000,
    "status": "APPROVED",
...
### 5️⃣ DELETE /api/procurement-contracts/{id} - 删除合同

```bash
curl -X DELETE http://localhost:3000/api/procurement-contracts/CONTRACT_ID
```
...
```json
{
  ...
  "data": [
    ...
    },
    {
      ...
      "code": "PROC1710710500000",
      "projectName": "项目 B",
      "constructionName": "主体施工",
      "supplierName": "供应商 B",
      "contractAmount": 200000,
      "payableAmount": 200000,
      "paidAmount": 50000,
      "unpaidAmount": 150000,
      "signDate": "2026-03-18T00:00:00Z",
...
- 使用索引加速查询 (projectId, constructionId, supplierId, code)
- 支持按条件过滤，减少返回数据量
